...
function collectionStatusLabel(status?: string) { return ({ SUCCESS: '采集成功', VERIFICATION_REQUIRED: '等待人工验证', LOGIN_REQUIRED: '需要登录', BLOCKED: '平台阻断', DOM_CHANGED: '页面结构变化', PARTIAL: '部分完成', FAILED: '采集失败' } as Record<string, string>)[status || ''] || status || '未记录' }
function collectionStatusTone(status?: string) { return status === 'SUCCESS' ? 'green' : ['PARTIAL', 'VERIFICATION_REQUIRED', 'LOGIN_REQUIRED'].includes(status || '') ? 'amber' : ['BLOCKED', 'DOM_CHANGED', 'FAILED'].includes(status || '') ? 'red' : 'neutral' }
...
function LeadDrawer({ lead, close, project, onUpdated }: RecordShape) {
  ...
  useEffect(() => {
    ...
    if (!lead.id) return
    request(`/api/leads/${lead.id}/follow-tasks?project_id=${project.id}`).then((items) => setFollowTasks(Array.isArray(items) ? items : [])).catch((error) => setFollowTasksError(errorText(error))).finally(() => setFollowTasksLoading(false))
  }, [lead.id, lead.status, lead.follow_note])
  async function generate() { if (!lead.id || typeof lead.id !== 'number') return; setLoading(true); setAdviceError(''); try { setAdvice(await request(`/api/leads/${lead.id}/assistant?project_id=${project.id}`, { method: 'POST' })) } catch (error) { setAdviceError(errorText(error)) } finally { setLoading(false) } }
  async function changeStatus(next: string) { setStatusBusy(true); setStatusMessage('保存中…'); try { const updated = await request(`/api/leads/${lead.id}?project_id=${project.id}`, { method: 'PATCH', body: JSON.stringify({ status: next }) }); setStatus(updated.status); onUpdated?.(updated); setStatusMessage('状态已更新') } catch (error: any) { setStatus(lead.status || 'NEW'); setStatusMessage(error.message) } finally { setStatusBusy(false) } }
  async function saveFollowNote() { setNoteBusy(true); setStatusMessage('保存备注中…'); try { const updated = await request(`/api/leads/${lead.id}?project_id=${project.id}`, { method: 'PATCH', body: JSON.stringify({ follow_note: followNote }) }); setFollowNote(updated.follow_note || ''); onUpdated?.(updated); setStatusMessage('人工备注已保存') } catch (error) { setStatusMessage(errorText(error)) } finally { setNoteBusy(false) } }
...
function RepliesView({ project, providerContext }: RecordShape) {
  ...
  const sendBatch = async () => {
    ...
    try {
      ...
      await reload()
    } catch (err: any) { setError(errorText(err)); await reload() } finally { setBusy(null) }
  }
  ...
  const sentItems = items.filter((item) => sentStatuses.includes(String(item.status)))
  const todayComments = comments.filter((item) => isToday(item.created_at || item.created_at_platform)).length
  const analysedComments = comments.filter((item) => item.intent_level || item.lead_id).length
  const leadComments = comments.filter((item) => item.lead_id).length
  const todayReplies = sentItems.filter((item) => isToday(item.sent_at || item.updated_at || item.created_at)).length
...
function LegacyKnowledgeView({ project }: RecordShape) {
  ...
  const save = async () => { if (!editingId || !title.trim() || !content.trim()) { setError('标题和事实内容不能为空'); return } try { const current = items.find((item) => item.id === editingId); await request(`/api/knowledge/${editingId}?project_id=${project.id}`, { method: 'PUT', body: JSON.stringify({ title: title.trim(), content: content.trim(), tags: current?.tags || [], enabled: current?.enabled !== false }) }); setEditingId(null); setTitle(''); setContent(''); await reload() } catch (err: any) { setError(err.message) } }
  const toggle = async (item: RecordShape) => { try { const updated = await request(`/api/knowledge/${item.id}?project_id=${project.id}`, { method: 'PUT', body: JSON.stringify({ title: item.title, content: item.content, tags: item.tags || [], enabled: item.enabled === false }) }); setItems((current) => current.map((row) => row.id === item.id ? updated : row)) } catch (err: any) { setError(err.message) } }
  const remove = async (id: number) => { if (!window.confirm('确认删除这条知识？')) return; try { await request(`/api/knowledge/${id}?project_id=${project.id}`, { method: 'DELETE' }); await reload() } catch (err: any) { setError(err.message) } }
...
function KnowledgeView({ project }: RecordShape) {
  ...
  const save = async () => {
    ...
    try { const current = editingId ? items.find((item) => item.id === editingId) : undefined; await request(editingId ? `/api/knowledge/${editingId}?project_id=${project.id}` : `/api/projects/${project.id}/knowledge`, { method: editingId ? 'PUT' : 'POST', body: JSON.stringify({ title: title.trim(), content: content.trim(), tags: current?.tags || [], enabled: current?.enabled !== false }) }); resetEditor(); setNotice(editingId ? '知识条目已更新' : '知识条目已添加'); await reload() } catch (err) { setError(errorText(err)) } finally { setBusy(false) }
  }
  const edit = (item: RecordShape) => { setEditingId(item.id); setTitle(item.title || ''); setContent(item.content || ''); setNotice('') }
  const toggle = async (item: RecordShape) => { setBusy(true); try { const updated = await request(`/api/knowledge/${item.id}?project_id=${project.id}`, { method: 'PUT', body: JSON.stringify({ title: item.title, content: item.content, tags: item.tags || [], enabled: item.enabled === false }) }); setItems((current) => current.map((row) => row.id === item.id ? updated : row)); setNotice(updated.enabled ? '知识条目已启用' : '知识条目已停用') } catch (err) { setError(errorText(err)) } finally { setBusy(false) } }
  const remove = async (id: number) => { if (!window.confirm('确认删除这条知识？')) return; setBusy(true); try { await request(`/api/knowledge/${id}?project_id=${project.id}`, { method: 'DELETE' }); if (editingId === id) resetEditor(); setNotice('知识条目已删除'); await reload() } catch (err) { setError(errorText(err)) } finally { setBusy(false) } }
...
function TasksView({ project, providerContext }: RecordShape) {
  ...
  useEffect(() => {
    // Use the replayable stream for immediate updates while retaining the
    // durable poll above as a fallback when the browser or proxy reconnects.
    let refreshScheduled = false
    let refreshTimer: number | undefined
    const queueRefresh = () => {
      if (refreshScheduled) return
      refreshScheduled = true
      refreshTimer = window.setTimeout(() => { refreshScheduled = false; void refresh(false) }, 250)
    }
    const stream = new EventSource(`/api/events/stream?project_id=${encodeURIComponent(String(project.id))}`)
    stream.onopen = () => setEventStatus('SSE 已连接')
    stream.onmessage = (event) => {
      try {
        const data = JSON.parse(event.data) as RecordShape
        if (data.message) setTaskNotice(`实时事件：${data.message}`)
        queueRefresh()
      } catch {
        // Malformed application frames remain covered by the poll fallback.
      }
    }
    stream.onerror = () => setEventStatus('SSE 重连中…')
    return () => { stream.close(); if (refreshTimer !== undefined) window.clearTimeout(refreshTimer); setEventStatus('SSE 已断开') }
  }, [project.id])
  ...
  useEffect(() => {
    ...
    let stopped = false
    const load = async () => { try { const detail = await request(`/api/tasks/${selectedTaskId}?project_id=${project.id}`); if (!stopped) { setTaskDetail(detail); setDetailError('') } } catch (err: any) { if (!stopped) setDetailError(err.message) } }
    void load()
  ...
  const followTaskAction = async (task: RecordShape, status: 'DONE' | 'CANCELLED') => {
    ...
    setFollowTasksError('')
    try {
      const updated = await request(`/api/follow-tasks/${task.id}?project_id=${project.id}`, { method: 'PATCH', body: JSON.stringify({ status }) })
  ...
  const markReminderRead = async (reminder: RecordShape) => {
    ...
    setFollowTasksError('')
    try {
      await request(`/api/follow-task-reminders/${reminder.id}/read?project_id=${project.id}`, { method: 'PATCH' })
  ...
  return (
    <div className="page">
      ...
      {selectedTaskId && <section className="panel data-panel task-detail">
        ...
        {detailError && <div className="error-inline"><span>{detailError}</span><Button onClick={() => { setDetailError(''); setTaskDetail(undefined); void request(`/api/tasks/${selectedTaskId}?project_id=${project.id}`).then(setTaskDetail).catch((err) => setDetailError(errorText(err))) }}>重试</Button></div>}
        {taskDetail ? <div className="task-detail-body">
          <div className="task-detail-summary"><span>任务状态 <b>{taskStatusLabel(taskDetail.task?.status)}</b></span><span>采集结果 <StatusPill tone={collectionStatusTone(taskDetail.report?.metrics?.collection_status)}>{collectionStatusLabel(taskDetail.report?.metrics?.collection_status)}</StatusPill></span><span>当前步骤 <b>{taskDetail.task?.current_step || '—'}</b></span><span>错误 <b>{taskDetail.task?.error || '无'}</b></span></div>
...
function LegacyLeadsView({ project }: RecordShape) {
  ...
  useEffect(() => { void reload() }, [project.id])
  const openLead = async (lead: RecordShape) => { try { setSelected(await request(`/api/leads/${lead.id}?project_id=${project.id}`)) } catch (err: any) { setError(err.message); setSelected(lead) } }
  ...
  const tabs = [{ key: 'all', label: '全部' }, { key: 'S', label: 'S 级' }, { key: 'A', label: 'A 级' }, { key: 'B', label: 'B 级' }, { key: 'C', label: 'C 级' }, { key: 'NEW', label: '待跟进' }]
  const counts: Record<string, number> = { all: items.length, S: items.filter((item) => item.lead_level === 'S').length, A: items.filter((item) => item.lead_level === 'A').length, B: items.filter((item) => item.lead_level === 'B').length, C: items.filter((item) => item.lead_level === 'C').length, NEW: items.filter((item) => item.status === 'NEW').length }
...
function LeadsView({ project }: RecordShape) {
  ...
  const openLead = async (lead: RecordShape) => { try { setSelected(await request(`/api/leads/${lead.id}?project_id=${project.id}`)) } catch (err) { setError(errorText(err)); setSelected(lead) } }
  const updateSelected = (updated: RecordShape) => { setSelected((current) => ({ ...current, ...updated })); setItems((current) => current.map((item) => item.id === updated.id ? { ...item, ...updated } : item)) }
  const tabs = [{ key: 'all', label: '全部' }, { key: 'S', label: 'S 级' }, { key: 'A', label: 'A 级' }, { key: 'B', label: 'B 级' }, { key: 'C', label: 'C 级' }, { key: 'NEW', label: '待跟进' }]
  const counts: Record<string, number> = { all: items.length, S: items.filter((item) => item.lead_level === 'S').length, A: items.filter((item) => item.lead_level === 'A').length, B: items.filter((item) => item.lead_level === 'B').length, C: items.filter((item) => item.lead_level === 'C').length, NEW: items.filter((item) => item.status === 'NEW').length }
...
function KeywordsViewLive({ project }: RecordShape) {
  ...
  useEffect(() => { void reload() }, [project.id])
  const categoryOrder = ['核心词', '需求词', '购买意向', '痛点词', '问题词', '价格词', '对比词', '避坑词', '竞品词', '地域词', '场景词', '人群词', '长尾词']
  const categories = ['全部', ...categoryOrder.filter((category) => items.some((item) => item.category === category)), ...Array.from(new Set(items.map((item) => item.category).filter((category) => category && !categoryOrder.includes(category))))]
  const rows = items.filter((item) => (category === '全部' || item.category === category) && (!query.trim() || String(item.keyword).includes(query.trim())))
  const toggle = async (item: RecordShape) => { try { const updated = await request(`/api/keywords/${item.id}?enabled=${!item.enabled}&project_id=${project.id}`, { method: 'PATCH' }); setItems((current) => current.map((row) => row.id === item.id ? updated : row)) } catch (err: any) { setError(err.message) } }
...
function VideosViewLive({ project, providerContext }: RecordShape) {
  ...
  const openDetails = async (item: RecordShape) => { setSelected(item); setDetail(undefined); setDetailLoading(true); try { const [video, comments] = await Promise.all([request(`/api/videos/${item.id}?project_id=${project.id}`), request(`/api/comments?project_id=${project.id}&limit=500`)]); setDetail({ video, comments: comments.filter((comment: RecordShape) => comment.video_id === item.id) }); setError('') } catch (err) { setError(errorText(err)) } finally { setDetailLoading(false) } }
  const syncComments = async () => {
    if (!selected?.id) return
    const capabilityMessage = providerCapabilityMessage(providerContext, commentRequirements)
    if (capabilityMessage) { setError(capabilityMessage); return }
    setSyncBusy(true); setError(''); setNotice('')
    try {
      ...
      while (true) {
        ...
        if (!result.next_cursor) throw new Error('评论分页声明还有更多数据但没有返回游标')
        if (seenCursors.has(result.next_cursor)) throw new Error('评论分页游标未前进，请重试同步')
        seenCursors.add(result.next_cursor)
        cursor = result.next_cursor
      }
      await openDetails(selected)
      setNotice(`评论同步完成：读取 ${received} 条，新增 ${created} 条，更新 ${updated} 条，共 ${pages} 页`)
    } catch (err) { setError(errorText(err)) } finally { setSyncBusy(false) }
  }
