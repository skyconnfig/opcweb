...
- 可观测性：总览接入 `/api/dashboard` 与 `/api/events/stream`；智能截流接入创建项目、smart-mode、scan；潜客抽屉接入 LeadAssistantAgent 建议接口。
- 内容边界：页面明确显示 Douyin Playwright 真实 Provider、评论覆盖范围以 DOM 返回为准，以及 LeadAssistantAgent 只生成建议、不自动发送。
...
4. 为潜客详情加入 AI 摘要、购买信号、评分信息和人工跟进建议抽屉；并接入人工备注与带截止时间的跟进任务（创建、完成、取消）。
5. 修复执行生产构建后旧 Next 开发进程引用失效 CSS/JS 的问题：`start.ps1` 会探测首页实际引用的全部静态资源，必要时只重启本项目 Web 进程。
6. 修复 hash 深链接和同文档导航切换不更新页面视图的问题，并为评论、视频和潜客详情抽屉补充 dialog 语义。
...
- 交互验证覆盖智能截流表单、评论/视频/潜客/任务详情抽屉、跟进任务读取、LeadAssistantAgent 建议入口、全部页面/hash 路由和移动端导航。

## Verification

- `.venv\\Scripts\\python.exe -m compileall -q backend tests`：通过
- `.venv\\Scripts\\python.exe -m pytest -q`：145 passed（2 个 Alembic 弃用警告）
- `npm run lint`：通过
- `npm run build`：通过
- `.venv\\Scripts\\python.exe scripts\\frontend_smoke_test.py`：15 个页面、15 个 hash、详情面板和移动端检查通过；console error 0、非 SSE 请求失败 0
