...
- PersonaAgent：项目、人设、潜客字段和评论文本 → 仅供人工确认的人设建议。
- ReplyAgent：项目、人设、潜客字段、评论文本和知识库 → 仅供人工确认的回复草稿。
- LeadAssistantAgent：行业/业务上下文、潜客字段、一级与二级评论、客户历史评论、知识库和人设 → `reply`、`reason`、`risk`、`next_action` 四个文本字段；仅供人工确认，不自动发送。
...
行业信息 → 文本行业理解 → 100–300 个关键词 → 文本元数据机会评分 → 公开评论 → 文本潜客判断 → 潜客池 → LeadAssistantAgent 销售跟进建议。
